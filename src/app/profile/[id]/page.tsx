...
import { FaCalendar, FaEdit, FaQuestion, FaUser, FaVoicemail, FaWhatsapp } from 'react-icons/fa'
...
const Page = (props: Props) => {
  ...
  const { data, isFetching, isError } = useGetUserByid(parseInt(params.id));
  ...
  const Postlist = (props: { name: string }) => {
    ...
    return (
      <div>
     <h1 className='my-2 text-xl font-bold block'>Posts by {props.name}</h1>
        {
          !isFetching && posts ?
          
          <div className='grid grid-cols-2 lg:grid-cols-3 gap-2'>
            {
             logedinuserdetails?.id != userdetails?.id? posts.map((e: Post) => (
              ...
              :posts.map((e: Post) => (
                <Listitem title={e.title} description={e.description} id={e.id} categoryId={e.categoryId} price={e.price} images={e.images} createdAt={e.createdAt} address={e.Address} key={e.id} showoptionmenu={(logedinuserdetails?.id === userdetails?.id)} onoptionclick={handledelete}/>
              ))
            }
          </div> 
          
          : <div className='flex justify-center items-center w-[100vw]'>Loading...</div>
  ...
  return (
    ...
    <div className='max-container padding-container'>
        ...
        {isError ? (
          ...
          <div className=''>
            ...
            <div className='flex items-center gap-x-1'>
              <FaQuestion size={12}/>username: {userdetails.username}
            </div>
            <div className='flex items-center gap-x-1'>
              ...
              </div>:  <div className=' gap-2 bg-green-600 border-black flex items-center justify-center rounded-md h-12 mt-2 cursor-pointer'>
      ...
      <div className='flex w-full bg-black h-0.5'>
    ...
    {
     showeditmodal && <EditProfileModal userdata={{name:userdetails?.name,image:userdetails?.image?userdetails.image:""}} setshoweditmodal={setshoweditmodal}/>
    }
  </>
  )
}
